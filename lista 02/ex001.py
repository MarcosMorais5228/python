...
elif(palavras_diferentes > 1) and (primeira_palavra == ultima_palavra):
    print(f'Parece que ocorreram {palavras_diferentes-1} trocas durante o processo, mas mesmo com essa pequena confusão, a palavra {primeira_palavra} conseguiu chegar no fim do telefone sem fio.')

if(palavras_diferentes == 2) and (primeira_palavra != ultima_palavra):
    print(f'Poxa, foi por pouco, só quem errou foi {jogador_errou} que disse {palavra_errada} ao invés de {primeira_palavra}…')
elif(palavras_diferentes == 3) and (primeira_palavra != ultima_palavra):
    ...
    print(f'É, parece que os alunos se confundiram bastante durante a brincadeira e a palavra {primeira_palavra} acabou virando {ultima_palavra}. No total, ocorreram {palavras_diferentes-1} trocas.')